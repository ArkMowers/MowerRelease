...
目前为 Windows x64、Linux x64 和 Linux ARM64 的独立包生成文件级 OTA。macOS DMG 仍使用完整包；Android 宿主的 OTA 接入在独立的 Android 仓库推进。发布流程从主仓库已发布的完整包构建最近五个旧版本直达当前版本的差异包。差异包达到完整包 85% 大小时不发布，客户端改用完整包。
