...
开发版使用 `vX.Y.Z-alpha.N.g<8 位提交短码>` 标记，例如 `v4.1.6-alpha.9.g40ac54e4`。本仓库每天北京时间 06:00 检查主仓库 `alpha` 分支；提交变化时，构建 Windows x64 和 Android ARM64 完整包，并直接发布到本仓库的 Release，随后生成 OTA 和独立的 `version/dev.json` 索引。主仓库不发布 nightly Release。每个 nightly 最多从五个旧开发版、两个普通公测版和两个正式版构建直达 OTA；测试版也保留最近两个正式版作为 OTA 起点。正式版索引只在新正式版发布后更新，测试版和开发版的发布不会触发正式版渠道更新。

OTA 清单 `ota.json` 包含起点、目标、平台、目标文件摘要及差异文件名。`payload/` 只存新增或变化的文件；不在目标清单中的旧文件自然删除。Android 的 `python-runtime.zip.xz` 还会按内部文件比较，只在 `runtime/` 携带变化的运行时文件。安装器在新目录重建、逐文件校验，再替换程序。当前文件不匹配、下载失败或校验失败时改用同一 Release 的完整包。
